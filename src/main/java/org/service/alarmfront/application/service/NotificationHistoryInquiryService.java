...
public class NotificationHistoryInquiryService implements InquireNotificationHistoryUseCase {
    ...
    @Override
    public Page<NotificationHistoryResponseDTO> getNotificationHistory(String customerId, int page, int size) {
        log.info("Retrieving notification history for customer: {}, page: {}, size: {}", customerId, page, size);
        LocalDateTime threeMonthsAgo = LocalDateTime.now().minusMonths(3);
        Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "createdAt"));

        return notificationRequestRepository.findNotificationHistoryByTargetId(customerId, threeMonthsAgo, pageable);
    }
}
